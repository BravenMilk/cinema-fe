import React from 'react';
import { getAllCities } from "../../../../api/global/Cities/CityApi";
import CustomSelect from '../../../../components/Common/CustomSelect';

export default function CinemaForm({ initialData = null, onSubmit, onCancel, loading = false }) {
    const [formData, setFormData] = React.useState({
        name: initialData?.name || '',
        address: initialData?.address || '',
        city_id: initialData?.city_id || ''
    });

    const [cities, setCities] = React.useState([]);
    const [errors, setErrors] = React.useState({});
    const [loadingCities, setLoadingCities] = React.useState(false);

    React.useEffect(() => {
        const fetchCitiesList = async () => {
            setLoadingCities(true);
            try {
                const response = await getAllCities({ limit: 100 });
                const cityData = response.data?.data || response.data || [];
                setCities(Array.isArray(cityData) ? cityData : []);
                console.log("CinemaForm: Cities loaded:", cityData);
            } catch (err) {
                console.error("CinemaForm: Gagal memuat daftar kota", err);
            } finally {
                setLoadingCities(false);
            }
        };
        fetchCitiesList();
    }, []);

    const handleSubmit = (e) => {
        e.preventDefault();
        const newErrors = {};
        if (!formData.name.trim()) newErrors.name = "Nama bioskop wajib diisi";
        if (!formData.address.trim()) newErrors.address = "Alamat wajib diisi";
        if (!formData.city_id) newErrors.city_id = "Kota wajib dipilih";

        if (Object.keys(newErrors).length > 0) {
            setErrors(newErrors);
            return;
        }

        onSubmit(formData);
    };

    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
                <label className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] ml-1">Nama Bioskop</label>
                <input
                    type="text"
                    value={formData.name}
                    onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                    placeholder="Contoh: Grand Indonesia XXI"
                    className={`w-full px-6 py-4 bg-white/5 border ${errors.name ? 'border-rose-500/50' : 'border-white/10'} rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500/40 transition-all text-white font-medium`}
                    disabled={loading}
                />
                {errors.name && <p className="text-rose-400 text-[10px] font-bold uppercase tracking-widest ml-1 italic">{errors.name}</p>}
            </div>

            <CustomSelect
                label="Pilih Kota"
                options={cities}
                value={formData.city_id}
                onChange={(val) => setFormData({ ...formData, city_id: val })}
                placeholder="Cari dan Pilih Kota Pengoperasian"
                loading={loadingCities}
                error={errors.city_id}
            />

            <div className="space-y-2">
                <label className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] ml-1">Alamat Lengkap</label>
                <textarea
                    value={formData.address}
                    onChange={(e) => setFormData({ ...formData, address: e.target.value })}
                    placeholder="Masukkan alamat lengkap bioskop..."
                    rows="3"
                    className={`w-full px-6 py-4 bg-white/5 border ${errors.address ? 'border-rose-500/50' : 'border-white/10'} rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500/40 transition-all text-white font-medium resize-none`}
                    disabled={loading}
                />
                {errors.address && <p className="text-rose-400 text-[10px] font-bold uppercase tracking-widest ml-1 italic">{errors.address}</p>}
            </div>

            <div className="flex items-center gap-4 pt-4">
                <button
                    type="button"
                    onClick={onCancel}
                    className="flex-1 py-4 bg-white/5 border border-white/10 text-slate-400 font-black rounded-2xl hover:bg-white/10 transition-all uppercase tracking-widest text-[10px]"
                    disabled={loading}
                >
                    Batalkan
                </button>
                <button
                    type="submit"
                    className="flex-[2] py-4 bg-indigo-600 text-white font-black rounded-2xl hover:bg-indigo-500 transition-all shadow-xl shadow-indigo-600/20 uppercase tracking-widest text-[10px] flex items-center justify-center disabled:opacity-50"
                    disabled={loading}
                >
                    {loading ? 'Menyimpan...' : initialData ? 'Update Bioskop' : 'Simpan Bioskop'}
                </button>
            </div>
        </form>
    );
}
