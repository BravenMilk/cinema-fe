...
import { Plus, Search, Edit3, Trash2, MonitorPlay, RefreshCcw, Loader2, Clapperboard, Layers,ChevronLeft,ChevronRight} from 'lucide-react';
import BaseModal from '../../../components/Common/BaseModal';
import NotificationModal from '../../../components/Common/NotificationModal';
import HallForm from './components/HallForm';
import { useAdminHalls } from '../../../hooks/admin/Halls/useAdminHalls';
import { getAllCinemas } from '../../../api/global/Cinemas/CinemaApi';
import CustomSelect from '../../../components/Common/CustomSelect';

export default function HallManager() {
    const { halls, loading, meta, page, setPage, search, setSearch, cinema_id, setCinemaId, handleCreate, handleUpdate, handleDelete, refresh} = useAdminHalls();
    ...
    React.useEffect(() => {
        const fetchCinemas = async () => {
            ...
            try {
                ...
                const sortedCinemas = Array.isArray(data) ? data : [];
                setCinemas([{ id: "", name: "Tampilkan Semua" }, ...sortedCinemas]);
            } catch (err) {
    ...
    const openDeleteConfirm = (hall) => {
        setHallToDelete(hall);
        setIsDeleteConfirmOpen(true);
    };

    const onFormSubmit = async (data) => {
        const result = selectedHall
            ? await handleUpdate(selectedHall.id, data)
        ...
        if (result.success) {
            ...
            setNotif({
                isOpen: true,
                type: 'success',
                title: selectedHall ? 'Update Berhasil' : 'Studio Ditambahkan',
                message: `Studio ${data.name} telah berhasil ${selectedHall ? 'diperbarui' : 'disimpan'} ke sistem.`
            });
        } else {
            setNotif({
                isOpen: true,
    ...
    const onConfirmDelete = async () => {
        const result = await handleDelete(hallToDelete.id);
        ...
        if (result.success) {
            setNotif({
                ...
                type: 'success',
                title: 'Dihapus',
                message: `Studio ${hallToDelete.name} telah berhasil dihapus.`
            });
        } else {
            setNotif({
                isOpen: true,
    ...
    const getTypeBadgeClass = (type) => {
        switch (type) {
            case 'IMAX': return 'bg-blue-500/10 text-blue-400 border-blue-500/20';
            case 'Premiere': return 'bg-amber-500/10 text-amber-400 border-amber-500/20';
            case '3D': return 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20';
    ...
    return (
        <div className="p-6 md:p-10 min-h-screen ">
            <div className="flex flex-col gap-3 mb-10">
                <div className='flex flex-col gap-3'>
                    ...
                    <div className="w-full">
                        <CustomSelect options={cinemas} value={cinema_id} onChange={(val) => { setCinemaId(val); setPage(1); }} placeholder="Semua Bioskop" loading={loadingCinemas}/>
                    </div>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-black/20">
                                ...
                                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-white/5">Studio / Hall</th>
                                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-white/5">Bioskop</th>
                                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-white/5 text-center">Tipe</th>
                            ...
                            {loading && halls.length === 0 ? (
                                <tr>
                                    <td colSpan="5" className="px-8 py-20 text-center">
                                        ...
                                        <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest animate-pulse">Scanning Architecture...</p>
                                    </td>
                                </tr>
                            ) : halls.length === 0 ? (
                                <tr>
                                    <td colSpan="5" className="px-8 py-20 text-center">
                                        <div className="text-slate-700 font-medium italic text-sm">Belum ada studio yang dikonfigurasi.</div>
                                    </td>
                                </tr>
                            ) : (
                                halls.map((hall, index) => (
                                    <tr key={hall.id} className="group hover:bg-white/[0.02] transition-colors border-b border-white/5">
                                        ...
                                        <td className="px-8 py-6">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs font-bold text-slate-400 uppercase tracking-widest group-hover:text-slate-200 transition-colors">
                                                    {hall.cinema?.name || 'N/A'}
                                                </span>
                                            </div>
                                        </td>
                                        <td className="px-8 py-6 text-center">
                                            <span className={`px-4 py-1.5 rounded-full border text-[9px] font-black uppercase tracking-widest ${getTypeBadgeClass(hall.type)}`}>
                                                {hall.type}
                                            </span>
                                        </td>
                                        <td className="px-8 py-6">
            ...
            <BaseModal isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} title={selectedHall ? 'Edit Konfigurasi Studio' : 'Tambah Studio Baru'} maxWidth="max-w-xl">
                <HallForm initialData={selectedHall} onSubmit={onFormSubmit} onCancel={() => setIsFormOpen(false)} loading={loading}/>
            </BaseModal>

            <NotificationModal isOpen={isDeleteConfirmOpen} onClose={() => setIsDeleteConfirmOpen(false)} type="warning" title="Konfirmasi Hapus" message={`Anda akan menghapus studio ${hallToDelete?.name}. Seluruh kursi dan jadwal tayang yang terhubung dengan studio ini akan ikut terhapus.`} actionLabel="Ya, Hapus Permanen" onAction={onConfirmDelete}/>
